local items = {
	...
	callBack = function(self)
		...
		s["header2"] = s.List:Add( "DPanel" )
		s["header2"]:SetSize( 790-10, 30 )
		s["header2"].Paint = function(self,w,h)
			draw.RoundedBox(0, 0, 0, w, h, Color(65,85,165));
			draw.SimpleTextOutlined( "Ящики", "F4BTN", w/2, h/2, Color(255,255,255,255), 1, 1, 1, Color(0,0,0,255) )
		end
		...
		for k,v in pairs(CustomShipments) do
		 	...
		 	if istable(v.allowed) and table.HasValue(v.allowed, ply:Team()) or !istable(v.allowed) and ply:Team() == v.allowed or v.allowed == nil then
				...
				s[name..k].OnClick = function()
					if v.noship then
						ply:ConCommand("DarkRP buy "..v.name)
					else
						ply:ConCommand("DarkRP buyshipment "..v.name)
					end
				end
			...
			elseif #CustomShipments == k and stat == 0 then
				s["header2"]:Remove()
			end

		end

		s["header3"] = s.List:Add( "DPanel" )
		s["header3"]:SetSize( 790-10, 30 )
		s["header3"].Paint = function(self,w,h)
			draw.RoundedBox(0, 0, 0, w, h, Color(85,65,165));
			draw.SimpleTextOutlined( "Патроны", "F4BTN", w/2, h/2, Color(255,255,255,255), 1, 1, 1, Color(0,0,0,255) )
		end
		...
		for k,v in pairs(GAMEMODE.AmmoTypes) do
			...
			s[name..k]:SetSize(790-10, 32)
			s[name..k].OnClick = function()
				ply:ConCommand("DarkRP buyammo "..k)
			end
